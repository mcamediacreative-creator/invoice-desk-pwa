...
const CLOUD_LAST_UID_KEY = "invoiceDeskPwa.lastCloudUid";
...
function loadState() {
  try {
    ...
    return { ...defaultState(), ...loaded, workingDocs: loaded.workingDocs || {} };
  } catch (err) {
...
function normalizeState(input) {
  ...
  const next = { ...base, ...input, workingDocs: input?.workingDocs || {} };
...
function cloudDataRef() {
  ...
  return cloudSync.db.collection("users").doc(cloudSync.uid).collection("apps").doc(CLOUD_APP_ID);
}
...
function setAuthLoading(loading) {
  ...
  if (button) {
    ...
    button.textContent = loading ? "Please wait..." : authMode === "login" ? "Sign In" : "Create Account";
  }
...
function updateAuthModeUi() {
  ...
  const reset = $("#authResetBtn");
  const password = $("#authPassword");

  if (title) title.textContent = isRegister ? "Create Your Account" : "Sign In";
  if (subtitle) subtitle.textContent = isRegister
    ? "Create one account, then use the same email and password on all your devices."
    : "Sign in to open your invoice system and sync documents across devices.";
  if (submit) submit.textContent = isRegister ? "Create Account" : "Sign In";
  if (toggle) toggle.textContent = isRegister ? "Back to sign in" : "Create new account";
  if (reset) reset.classList.toggle("hidden", isRegister);
  if (password) password.setAttribute("autocomplete", isRegister ? "new-password" : "current-password");
}

function updateUserDisplay() {
  const emailNode = $("#userEmailLabel");
  const signOut = $("#signOutBtn");
  if (emailNode) {
    emailNode.textContent = cloudSync.email || "";
    emailNode.classList.toggle("hidden", !cloudSync.email);
  }
  if (signOut) signOut.classList.toggle("hidden", !cloudSync.email);
}
...
function firebaseErrorMessage(error) {
  ...
  const friendly = {
    ...
    "auth/invalid-login-credentials": "Wrong email or password. Please check and try again.",
    "auth/user-not-found": "No account found for this email. Create a new account first.",
...
async function flushCloudSave(force = false) {
  ...
  try {
    ...
    try { localStorage.setItem(CLOUD_LAST_UID_KEY, cloudSync.uid || ""); } catch (err) {}
    setSyncStatus("Synced", "synced");
  } catch (err) {
...
function recordTime(record = {}) {
  const value = record.updatedAt || record.createdAt || record.updatedAtLocal || "";
...
function hasMeaningfulData(candidate = {}) {
  ...
  const workingDocs = candidate.workingDocs && typeof candidate.workingDocs === "object" ? candidate.workingDocs : {};
  if (Object.keys(workingDocs).length) return true;
...
function mergeStatesForCloud(localState, cloudState) {
  ...
  const cloud = normalizeState(cloudState || defaultState());
  const merged = normalizeState({
    ...cloud,
    activeCompanyId: cloud.activeCompanyId || local.activeCompanyId,
    ...
    docs: mergeArrayById(local.docs, cloud.docs),
    workingDocs: mergeWorkingDocs(local.workingDocs, cloud.workingDocs)
  });
  if (!merged.companies.find(c => c.id === merged.activeCompanyId)) {
    merged.activeCompanyId = merged.companies[0]?.id || local.activeCompanyId || cloud.activeCompanyId;
  }
  return merged;
}
...
function chooseInitialCloudState(incomingState) {
  ...
  if (localHasUsefulData && !cloudHasUsefulData) return { state: normalizeState(state), shouldUpload: true, note: "Uploaded this device data to cloud." };
  if (localHasUsefulData && cloudHasUsefulData) {
    const merged = mergeStatesForCloud(state, incoming);
    return { state: merged, shouldUpload: true, note: "Merged this device with cloud data." };
  }
  return { state: incoming, shouldUpload: false, note: "Loaded cloud data." };
}
...
function applyCloudState(incomingState) {
  ...
  try {
    ...
    persistLocalStateOnly();
    cloudSync.lastAppliedJson = JSON.stringify(cleanForFirestore(state));
...
function startCloudListener(user) {
  ...
  updateUserDisplay();
  showAuthGate(true);
  setAuthMessage("Loading your cloud data...", "muted");
  setSyncStatus("Loading cloud...", "saving");
  ...
  cloudSync.unsubscribe = ref.onSnapshot(async snapshot => {
    try {
      if (!snapshot.exists) {
        if (!cloudSync.ready) {
          try {
            const lastUid = localStorage.getItem(CLOUD_LAST_UID_KEY);
            if (lastUid && lastUid !== cloudSync.uid) {
          ...
          showAuthGate(false);
          setAuthMessage("Cloud file created from this device.", "success");
          setSyncStatus("Creating cloud data...", "saving");
          renderAll();
      ...
      const incomingJson = JSON.stringify(cleanForFirestore(incoming));
      const currentJson = JSON.stringify(cleanForFirestore(state));
  ...
  try {
    ...
    cloudSync.auth.onAuthStateChanged(user => {
      if (!user) {
        ...
        updateAuthModeUi();
        setSyncStatus("Sign in", "neutral");
        setAuthMessage("Sign in to sync this app across every device.", "muted");
        return;
      }
...
async function handleAuthSubmit(event) {
  ...
  try {
    setAuthLoading(true);
    setAuthMessage(authMode === "login" ? "Signing in..." : "Creating your account...", "muted");
    if (authMode === "login") await cloudSync.auth.signInWithEmailAndPassword(email, password);
    else await cloudSync.auth.createUserWithEmailAndPassword(email, password);
  } catch (err) {
...
function toggleAuthMode() {
  ...
  updateAuthModeUi();
  setAuthMessage(authMode === "login" ? "Sign in to sync this app across every device." : "Create one account, then use it on all your devices.", "muted");
}

async function resetAuthPassword() {
  if (!cloudSync.auth) {
    setAuthMessage("Firebase is not ready yet. Refresh the page and try again.", "error");
    return;
  }
  const email = ($("#authEmail")?.value || "").trim();
  if (!email) {
    setAuthMessage("Enter your email first, then click Reset password.", "error");
    $("#authEmail")?.focus();
    return;
  }
  try {
    setAuthMessage("Sending password reset email...", "muted");
    await cloudSync.auth.sendPasswordResetEmail(email);
    setAuthMessage("Password reset email sent. Please check your inbox or spam folder.", "success");
  } catch (err) {
    setAuthMessage(firebaseErrorMessage(err), "error");
  }
}
...
function renderDocumentList() {
  ...
  }).join("") : `<tr><td colspan="7" class="muted">No document found for this company.</td></tr>`;
}
...
function saveDoc(type) {
  type = validDocType(type);
  const doc = normalizeDoc({ ...workingDoc(type), id: workingDoc(type).id || uid("doc"), updatedAt: new Date().toISOString() }, type, activeCompany().id);
  ...
  state.workingDocs[`${activeCompany().id}:${type}`] = makeDoc(type);
  saveState();
  flushCloudSave(true);
  renderAll();
  switchTab("listTab");
...
function bindEvents() {
  document.addEventListener("click", event => {
    ...
    const authModeToggle = target.closest("#authModeToggle");
    if (authModeToggle) { event.preventDefault(); toggleAuthMode(); return; }

    const authResetBtn = target.closest("#authResetBtn");
    if (authResetBtn) { event.preventDefault(); resetAuthPassword(); return; }
  ...
  $("#authModeToggle")?.addEventListener("click", event => { event.preventDefault(); event.stopPropagation(); toggleAuthMode(); });
  $("#authResetBtn")?.addEventListener("click", event => { event.preventDefault(); event.stopPropagation(); resetAuthPassword(); });

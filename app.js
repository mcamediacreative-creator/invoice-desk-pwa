...
function updateAuthModeUi() {
  ...
  if (subtitle) subtitle.textContent = "Enter your staff ID or name before opening the shared invoice system.";
...
function updateUserDisplay() {
  ...
  if (userNode) {
    userNode.textContent = cloudSync.operatorId ? `ID: ${cloudSync.operatorId}` : "";
...
function firebaseErrorMessage(error) {
  ...
  const friendly = {
    "auth/operation-not-allowed": "Anonymous sign-in is not enabled. Enable Anonymous in Firebase Authentication > Sign-in method.",
    "auth/network-request-failed": "Network error. Please check your internet connection.",
    "permission-denied": "Firestore permission denied. Please update your Firestore Rules for the shared app path.",
...
async function flushCloudSave(force = false) {
  ...
  try {
    ...
    try { localStorage.setItem(CLOUD_LAST_SCOPE_KEY, "shared"); } catch (err) {}
    setSyncStatus("Synced", "synced");
    updatePresenceNow();
  } catch (err) {
...
function chooseInitialCloudState(incomingState) {
  ...
  if (localHasUsefulData && !cloudHasUsefulData) return { state: normalizeState(state), shouldUpload: true, note: "Uploaded this device data to the shared cloud file." };
  ...
  return { state: incoming, shouldUpload: false, note: "Loaded the shared cloud file." };
}
...
function initFirebaseSync() {
  ...
  try {
    ...
    setAuthMessage("Enter your ID to access the shared synced documents.", "muted");
...
async function handleAuthSubmit(event) {
  ...
  const operatorId = normalizeOperatorId($("#authId")?.value || "");
  if (!operatorId) {
    setAuthMessage("Please enter your ID or name first.", "error");
    $("#authId")?.focus();
    return;
  }
  try {
    setAuthLoading(true);
    setAuthMessage("Opening the shared invoice system...", "muted");
    cloudSync.operatorId = operatorId;
    cloudSync.lastActivityAt = Date.now();
    const user = cloudSync.auth.currentUser || (await cloudSync.auth.signInAnonymously()).user;
    startCloudListener(user);
  } catch (err) {
    cloudSync.operatorId = "";

...
function startCloudListener(user) {
  ...
  cloudSync.unsubscribe = ref.onSnapshot(async snapshot => {
    try {
      ...
      if (!cloudSync.ready || (!isOwnSave && incomingJson !== currentJson)) {
...
function toggleAuthMode() {
  ...
  if (toggle) toggle.textContent = authMode === "login" ? "Create new account" : "I already have an account";
  if (password) password.setAttribute("autocomplete", authMode === "login" ? "current-password" : "new-password");
...
function handleQuickFormSubmit(event) {
  ...
  if (kind === "item") {
    const record = { id: id || uid("item"), name: formData.name, details: formData.details, unit: formData.unit || "pcs", price: number(formData.price) };
    const idx = state.items.findIndex(i => i.id === record.id);
    if (idx >= 0) state.items[idx] = record; else state.items.push(record);
  }
  if (kind === "customer") {
    const record = { id: id || uid("customer"), name: formData.name, phone: formData.phone, email: formData.email, address: formData.address };

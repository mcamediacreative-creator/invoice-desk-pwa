...
### Firestore rules
Use these Firestore rules so each login account can only access its own invoice app data:
...
When you sign in on the first device, the app checks Firestore. If no cloud data exists yet, the current local browser data is uploaded as your first cloud copy.

After that, sign in with the same email and password on every device to load the same data.

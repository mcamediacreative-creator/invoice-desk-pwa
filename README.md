...
After that, sign in with an approved ID and Access PIN / Password on another device to load the same shared data.
...
This build uses a simple **User ID + Access PIN / Password** page. The app converts the ID into a hidden Firebase Authentication email behind the scenes, then uses the custom display name for online status, edit history, and working-on-document indicators.

Important Firebase setup:
1. In Firebase Authentication, enable **Email/Password** sign-in provider.
...
Changes included:
- Users enter an approved User ID and Access PIN / Password.
- Only Firebase Authentication users that you create can sign in.
- Everyone who signs in shares the same synced cloud document set.
...
## Update - Approved ID Access List

This version blocks random visitors from entering the app with any name.

Login now uses:
- **User ID** shown to staff, for example `STAFF01`
- **Access PIN / Password** stored securely in Firebase Authentication
- **allowed-users.js** for the custom display name and role shown in online/editing/history panels

### How to add a user
1. Go to Firebase Console > Authentication > Sign-in method and enable **Email/Password**.
2. Go to Firebase Console > Authentication > Users > Add user.
3. Use this email format: `USERID@invoice-desk-pwa.app`.
   - Example: `STAFF02@invoice-desk-pwa.app`
4. Set the password/PIN there. Firebase requires at least 6 characters.
5. Open `allowed-users.js` and add the same ID with display name and role.
6. Upload `allowed-users.js` to GitHub and redeploy Firebase Hosting.

Example allowed-users.js entry:

```js
{ id: "STAFF02", name: "Ali Staff", role: "Staff" }
```

Important: do not store PINs inside `allowed-users.js`. PINs/passwords must be created in Firebase Authentication only.

### Stronger Firestore Rules for approved users only

Use these rules when you want only approved Firebase Authentication users to access the shared app. After creating a Firebase Auth user, copy that user's UID and create a Firestore document at:

`sharedApps/invoiceDeskPwa/members/USER_UID`

The document can contain anything simple, for example:
...
service cloud.firestore {
  match /databases/{database}/documents {
    function isApprovedMember(appId) {
      return request.auth != null
        && exists(/databases/$(database)/documents/sharedApps/$(appId)/members/$(request.auth.uid));
    }

    match /sharedApps/{appId} {
      allow read, write: if isApprovedMember(appId);
...
This means knowing the website link is not enough. The person must have a Firebase Authentication account and their UID must be added to the Firestore members list.


## Update - Refresh Login Persistence

This build keeps approved ID users logged in after refreshing the page during their active 1-hour session. The app stores only the approved ID session metadata locally; it does not store the PIN/password. Manual Sign Out and the 1-hour auto logout still clear the saved session.


## Update - Document-Level Sync

This build changes cloud sync so workspace choices stay local to each user/device. Selecting a company, switching tabs, and editing an unsaved draft no longer changes another user's screen. Shared cloud sync now focuses on saved records: companies, customers, items, saved documents, templates, wording, and document deletion markers.

Multiple users can work at the same time under different companies. Their saved documents will merge into the shared document list, while each user keeps their own selected company and working draft locally.
